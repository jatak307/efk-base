...
const renderGameField = (main: HTMLElement, categoryNum: number) => {
  ...
  field.innerHTML = '';
  const selectedCardsCategory = cardsItemsData.get(categories[categoryNum]);
  selectedCardsCategory?.map((c: CardContent) => field.insertAdjacentHTML('beforeend',
...
const finishedGame = (main: HTMLElement, mistakes: number) => {
  if (!main) return;
  if (mistakes === 0) {
    playSoundFunc('success.mp3');
    main.innerHTML = '<img src=\'./assets/images/you-win.jfif\'>';
    setTimeout(() => {
      renderMain();
    }, 5000);
  }
  if (mistakes > 0) {
    ...
    main.innerHTML = `<img src='./assets/images/you-lost.jfif'>
...
export const renderGameCards: (categoryNum: number) => void = (categoryNum: number) => {
  ...
  const voiceTheWord = () => {
    if (cardsForGame.length === 0) {
      finishedGame(main, mistakeCounter);
      newGameStatus = false;
      mistakeCounter = 0;
      return;
    }
